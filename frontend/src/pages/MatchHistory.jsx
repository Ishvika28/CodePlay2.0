    ...
    const fetchHistory = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/match-history`,
        );
        ...
        const filtered = data.filter((match) => {
          if (Array.isArray(match.participants)) {
            return match.participants.includes(currentUser);
          }

          return (
            match.winnerName === currentUser ||
            match.rankings?.some((player) => player.name === currentUser)
          );
        });

        const uniqueMatches = filtered.filter(
          (match, index, self) =>
            index === self.findIndex((m) => m.roomCode === match.roomCode),
        );
      ...
      <main className="p-6 md:p-12 relative z-10 max-w-7xl mx-auto">
        ...
        <section className="relative overflow-hidden rounded-[3rem] border border-white/5 bg-slate-900/20 backdrop-blur-3xl shadow-2xl p-10 md:p-14 mb-10">
          <div className="absolute top-0 right-0 opacity-10">
            <Trophy size={300} className="text-blue-500" />
          </div>
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-400 max-w-2xl leading-relaxed text-base">
              Analyze your previous coding battles, review rankings, and track
              your competitive performance across contests.
            </p>
          </div>
        </section>
        ...
        <section className="grid md:grid-cols-3 gap-6 mb-10">
          <div className="bg-[#0b1120]/50 border border-white/5 rounded-3xl p-5 backdrop-blur-xl">
            ...
            <h2 className="text-4xl font-black text-white">{matches.length}</h2>
          </div>
          ...
          <div className="bg-[#0b1120]/50 border border-white/5 rounded-3xl p-5 backdrop-blur-xl">
            ...
            <h2 className="text-4xl font-black text-yellow-400">
              {
                matches.filter(
                  (m) => m.winnerName === localStorage.getItem("name"),
                ).length
              }
            </h2>
          </div>
        ...
        {/* MATCHES */}
        {loading ? (
          <div className="text-slate-500">Loading battles...</div>
        ) : matches.length === 0 ? (
          <div className="bg-[#0b1120]/50 border border-white/5 rounded-[2rem] p-10 text-center">
            <h2 className="text-3xl font-black mb-3">No Battles Found</h2>
          ...
          <div className="w-full max-w-4xl bg-[#0f172a] border border-white/10 rounded-[2rem] p-8 relative shadow-2xl">
            ...
            <div className="overflow-hidden rounded-3xl border border-white/5">
              <table className="w-full">
                ...
                <tbody>
                  {selectedMatch.rankings.map((user, index) => (
                    <tr
                      key={index}
